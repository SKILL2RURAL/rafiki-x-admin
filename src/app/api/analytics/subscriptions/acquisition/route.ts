...
export async function GET(request: Request) {
  ...
  const backendUrl = new URL(
    `${BACKEND_API_URL}/api/admin/analytics/subscriptions/acquisition`
  );
  ...
  try {
    ...
    if (!apiRes.ok) {
      return NextResponse.json(
        { message: data.message || "Failed to fetch subscription acquisition" },
    ...
    console.error("Error fetching subscription acquisition:", error);
    return NextResponse.json(
